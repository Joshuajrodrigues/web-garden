...
1. We'll start with the oidc config
...
2. Since I like to use Zustand I will also keep a sync compoent to sync the context with the state manager.
...
3. In our Root, App.tsx we wrap the provider around the app
...
- FIX: use a dedicated redirect_uri: `${HOST_URL}/signin-callback` and `onSigninCallback` to Redirect the user to the dashboard (index) after successful login.
- FIX: Use the `hasAuthParams()` utility from `react-oidc-context`. This tells your app to wait and not navigate away.
...
- <u>Do I need like silient.html ? callback.html ? login.html ??</u>
    ...
    In modern SPAs, we now prefer Refresh Token Rotation + `offline_access`. This bypasses the iframe entirely and makes `automaticSilentRenew` use a standard POST request to the token endpoint instead.
